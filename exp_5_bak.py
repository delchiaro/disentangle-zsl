...
from disentangle.layers import get_fc_net, GradientReversal, GradientReversalFunction, get_1by1_conv1d_net, IdentityLayer, BlockLinear
...
class Model:
    def __init__(self,
                 ...
                 device=None):
        self.autoencoder = Autoencoder(feats_dim, ka_dim, kc_dim, nb_attributes, autoencoder_hiddens).to(device)
...
def exp(model: Model, train, test_unseen, test_seen=None, val=None,
        bs=128,
        use_masking=False, train_w_frankenstain=False, early_stop_patience=None,
   ...
   #  model.save(state_dir, epoch=0, acc=acc)
    ...
    decoder_params = flatten([list(m.parameters()) for m in [model.autoencoder.decK_X, # model.autoencoder.decZ_X,
                                               model.autoencoder.decKA, model.autoencoder.decKC, #model.autoencoder.decKA_disA
                                                             ]])
    encoder_params = flatten([list(m.parameters()) for m in
                              [#model.autoencoder.encX_Z,
                               model.autoencoder.encX_KA, model.autoencoder.encX_KC]])

    # *********** AUTOENCODER TRAINING
    opt = torch.optim.Adam(encoder_params #+ decoder_params
                             + list(model.classifier.parameters())
                             + list(model.cntx_classifier.parameters()), lr=a_lr)
    d_opt = torch.optim.Adam(decoder_params + list(model.classifier.parameters()) + list(model.cntx_classifier.parameters()), lr=a_lr*10)
    ...
    for ep in range(nb_epochs):
        ...
        #frnk_iter = iter(DataLoader(frnk_dataset, bs, shuffle=True, num_workers=0, drop_last=False, pin_memory=True))
        ...
        def losses(L, x, y, a, x1, a1, ad, ac, logits=None, logits1=None, logits_cntx=None):
            zero = torch.tensor([0.],).mean()
            ...
            l_rec_x = NN.l1_loss(x1, x)  # image reconstruction
            ...
            l_cls = NN.cross_entropy(logits, y) if logits is not None else zero
            l_cls1 = NN.cross_entropy(logits1, y) if logits1 is not None else zero
            l_cls_cntx = NN.cross_entropy(logits_cntx, y) if logits_cntx is not None else zero
        ...
        for i, (x, y) in enumerate(train_loader):
            x, y = x.to(model.device), y.to(model.device)
            a = A[y]
            # NB: Masking with continuous attributes seems to work better!


            # a_diag = a_dis[:, diag_idx[0], diag_idx[1]].reshape(a.shape[0], nb_attributes)

            opt.zero_grad()
            ka, kc, x1, a1, ad, ac = model.autoencoder.forward(x, A_mask[y] if use_masking else None)
            logits1 = model.classifier(x1)
            logits = model.classifier(x)
            logits_cntx = model.cntx_classifier(kc)
            losses(L, x, y, a, x1, a1, ad, ac, logits, logits1, logits_cntx).backward()
            opt.step()
            opt.zero_grad()

            if train_w_frankenstain:
                d_opt.zero_grad()
                frnk_ka, frnk_kc, frnk_y = [t.to(model.device).detach() for t in frnk_dataset.get_items_with_class(y)]
                frnk_a_mask = A_mask[frnk_y]
                frnk_x1 = model.autoencoder.decode(frnk_ka, frnk_kc, frnk_a_mask)
                frnk_a1, frnk_ad = model.autoencoder.attr_decode(frnk_ka, frnk_a_mask)
                frnk_ac = model.autoencoder.cntx_attr_decode(frnk_kc)
                #logits = model.classifier(x)
                #logits_cntx = model.cntx_classifier(kc)
                frnk_logits1 = model.classifier(frnk_x1)
                losses(frnk_L, x, y, a, frnk_x1, frnk_a1, frnk_ad, frnk_ac, logits1=frnk_logits1).backward()
                d_opt.step()
...
def gen_zsl_test(autoencoder: Autoencoder,
    ...
    def enc_fn(X):
        ...
        Z=X
        return autoencoder.encX_KA(Z), autoencoder.encX_KC(Z)
...
def main():
    ...
    _, _, test_accs = exp(model, train, test_unseen, test_seen, val,
                          ...
                          #train_w_frankenstain=True,

                          test_lr=.0004, test_gen_samples=200,  # 2000, 800
                          test_period=10, test_epochs=10,
