...
#     return np.vstack(X_contr), np.array(Y_contr)
    ...
    ##Inside DstentangleZSL Module:
    #
    # def reconstruct(self, feats_input, attrib):
    #     feats_input = torch.tensor(feats_input).float().to(self.device)
    #     attrib = torch.tensor(attrib).float().to(self.device)
    #     attr_enc, cntx_enc = self.encode(feats_input)
    #     decoded = self.decode(attr_enc, cntx_enc, attrib)
    ...
    #
    #     logits = self.classify(attr_enc_exp, cntx_enc_exp, all_attrs_exp)
    #     t = torch.tensor([[t] for t in list(range(nb_classes)) * bs]).to(device)
    #     logits_diags = torch.gather(logits, 1, t).view(bs, nb_classes)
    #     return logits_diags.argmax(dim=1)
    #     #softmax = NN.softmax(logits_diags, dim=1)
    #     #return softmax
    #
    # def naive_predict(self, X, Y, all_attrs):
    #     attr_enc, cntx_enc = self.encode(X)
    #     A = all_attrs[Y]
    #     #attr_enc_exp_masked = self.mask_attr_enc(attr_enc, A)
    #     logits = self.classify(attr_enc, cntx_enc, A)
