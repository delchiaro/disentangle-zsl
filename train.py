...
from test import run_zsl_test, TestCfg, run_test
...
def run_train(generator: DisentangleGen,
              train,
              zsl_test_unseen,
              gzsl_test_seen=None,
    ...
    A_train_all_bin = torch.tensor(train['class_attr_bin']).float().to(device)
    pretraining = 5
    from time import time

    if cfg.classifier_pretrain_epochs > 0:
        print("Pretraining classifier...")
        class_opt = Adam(generator.classifier.parameters(), lr=.0004)
        for ep in range(cfg.classifier_pretrain_epochs):
            mean_loss = []
            for X, Y in train_loader:
                X = X.to(device)
                Y = Y.to(device)
                logits = generator.classifier(X)
                l_class = NN.cross_entropy(logits, Y)
                l_class.backward()
                class_opt.step()
                mean_loss.append(l_class.detach().cpu())
            print(f"Pretraining classifier epoch [{ep+1}/{cfg.classifier_pretrain_epochs}] - loss: {torch.stack(mean_loss).mean()}")
        for p in generator.classifier.parameters():  # set requires_grad to False
    ...
    for ep in range(cfg.nb_epochs):
        ...
        l_reconstr = l_attr_reconstr = l_class = l_decoded_class = l_cntx_class = l_disentangle = torch.tensor([0.]).to(device)
        reconstruction_loss = []
        attribute_rec_loss = []
        classification_loss = []
        ...
        while i < len(train_loader):
            ...
            if ep < cfg.autoencoder_pretrain_epochs:
                ...
                #l = 0*l_reconstr + 10* l_attr_reconstr + 0 * l_class + 0 * l_cntx_class + 0 * l_disentangle
        ...
        if (ep + 1) == cfg.test_epoch or ((ep + 1) >= cfg.test_epoch and (ep + 1) % cfg.test_period == 0):
            ...
            run_zsl_test(generator, train, zsl_test_unseen,
                         test_cfg.nb_gen_class_samples,
                         test_cfg.adapt_epochs, test_cfg.adapt_lr, test_cfg.adapt_bs,
                         test_cfg.use_infinite_dataset,
                         attrs_key=attr_key,
                         device=device)
...
#         time_ep_begin = time()
#
#         l_reconstr = l_attr_reconstr = l_class = l_decoded_class = l_cntx_class = l_disentangle = torch.tensor([0.])
#         reconstruction_loss = []
#         attribute_rec_loss = []
#         classification_loss = []
#         decoded_classification_loss = []
...
#                 l_class = NN.cross_entropy(decoded_logits, Y)
#                 #l_class = NN.cross_entropy(logits, Y)
#                 l_cntx_class = NN.cross_entropy(cntx_logits, Y)
#                 l_disentangle = generator.disentangle_loss(attr_enc, cntx_enc, Y, A_train_all)
#                 l = 0*l_reconstr + 10* l_attr_reconstr + 0 * l_class + 0 * l_cntx_class + 0 * l_disentangle
